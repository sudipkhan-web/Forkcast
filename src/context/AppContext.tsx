...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/inventory`));
    ...
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/shoppingList`));
    ...
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/pantryLogs`));
    ...
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/ingredientRules`));
    ...
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/household`));

    const unsubGroups = onSnapshot(collection(db, `users/${userId}/groups`), (snapshot) => {
      setGroups(snapshot.docs.map(d => d.data() as Group));
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/groups`));
    ...
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/plannedMeals`));
    ...
    }, (e) => handleFirestoreError(e, OperationType.LIST, `users/${userId}/favorites`));
  ...
  const updateCustomIngredientRule = async (name: string, rule: { location: 'fridge' | 'pantry', category: string }) => {
    ...
    try {
      ...
      handleFirestoreError(error, OperationType.UPDATE, `users/${userId}/ingredientRules/${normalizedName}`);
    }
  };
  ...
  const markAllNotificationsAsRead = async () => {
    ...
    try {
      ...
      handleFirestoreError(error, OperationType.UPDATE, `users/${userId}/notifications/markAll`);
    }
  };
