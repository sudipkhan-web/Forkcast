...
export function FavoritesView({
  ...
  return (
    ...
    >
      <header className="px-6 py-4 flex items-center gap-4 bg-[#fdfbf7]/80 backdrop-blur-xl border-b border-stone-200/60 shrink-0 z-20 sticky top-0">
        <button 
          onClick={() => setActiveTab('home')}
          className="p-2 -ml-2 text-stone-400 hover:text-stone-900 transition-all active:scale-95"
        >
          <ChevronLeft className="w-6 h-6" />
        </button>
        <h1 className="text-2xl font-display font-bold text-stone-900 tracking-tight">Saved Recipes</h1>
      </header>
        ...
        {favorites.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-6">
            {favorites.map(meal => (
              <div 
                key={meal.id} 
                className="bg-white rounded-2xl shadow-sm border border-stone-200/60 overflow-hidden cursor-pointer hover:border-stone-400 transition-colors relative group"
                onClick={() => {
              ...
              >
                <button
                  ...
                  className="absolute top-2 right-2 p-1 bg-white/90 backdrop-blur-md rounded-full text-stone-400 hover:text-red-500 hover:bg-white transition-all active:scale-95 shadow-sm z-10 opacity-70 hover:opacity-100"
                ...
                <div className="p-3">
                  <h3 className="font-display font-bold text-stone-900 text-sm line-clamp-2">{meal.name}</h3>
