...
const newFields = `
            {/* Biometrics */}
            <section className="bg-stone-900 border border-stone-800 rounded-3xl p-6 shadow-sm">
  ...
  const regex = /<label className="text-xs font-medium text-stone-300 mb-1 block">Weekly Training Days<\/label>[\s\S]*?<\/select>\s*<\/div>\s*<\/div>\s*<\/section>/;
